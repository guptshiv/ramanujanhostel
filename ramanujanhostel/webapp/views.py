...
department = {'COE' : 'Computer Engineering',
			...
			'MPAE' : 'Manufacturing Processing and Automation Engineering'}
...
def warden(request):
	...
	if(user_id and password):
		...
		db = mdb.connect(user='root', db='ramanujan_test',  passwd='shivam123', host='localhost')
		cursor = db.cursor()
...
def verify_for_registration(request):
	...
	db = mdb.connect(user='root', db='ramanujan_test',  passwd='shivam123', host='localhost')
	cursor = db.cursor()
	if user_id and password and roll_no and dept and name:
		#password = conv_to_sha(password)
		...
		sql_query = "SELECT name FROM student WHERE roll_num =" + " '" + roll_no + "' AND department = '" + department[dept] + "';"
		cursor.execute(sql_query)
		names = [row[0] for row in cursor.fetchall()]
		items = len(names)
		# Verify if student has already registered in the ramanujan
		valid_stud = (items == 0)
		
		sql_query = "SELECT hostel_name FROM resides_in WHERE department = " + " '" + department[dept] + "';"
		cursor.execute(sql_query)
...
def verify_for_room(request):
	...
	if user_id and h_name and roll_no and dept and room_num:
		db = mdb.connect(user='root', db='ramanujan_test',  passwd='shivam123', host='localhost')
		cursor = db.cursor()

		# Check if room number is in range alloted by the administration
		sql_query = "SELECT range_start, range_end FROM resides_in WHERE hostel_name = " + "'" + h_name +" Hostel' " + "AND department = " +  "'" + department[dept] + "';"
		cursor.execute(sql_query)
		...
		sql_query = "SELECT number FROM room ;"
		cursor.execute(sql_query)
		db.close()
...
def complete_registration(request):
	...
	local_guardian_contact_num = request.POST.get('lg_contactnum')
	...
	guardian_info = local_guardian_name and local_guardian_contact_num and local_guardian_address
	#hostel_info = prev_h_name and prev_room_num
	# If all information has been provided
	if(guardian_info and contact_num and receipt and bank):
		valid_names = True
		for name_i in [name, local_guardian_name]:
		...
		print(full_rooms)
		if(int(room_num) in full_rooms):
			message = "The room is already occupied, please choose a different one"
			context_dict = {'user_id' : user_id,
			'roll_no' : roll_no,
			'dept' : dept,
			'valid_id' : True,
		...
		sql_query += receipt + "', '" + contact_num + "', '" + department[dept] + "', 'B.E.', '1', '"
		print(sql_query)
		print(local_guardian_name,local_guardian_contact_num,local_guardian_address)
...
def stud_info(request):
	if(request.session.get('warden')):
		...
		sql_query = "SELECT number from room WHERE roll_num = " + "'" + roll_no + "';"
		#print(sql_query)
		cursor.execute(sql_query)
		details1 = cursor.fetchall()
		#print(details1)
		ans1 = [detail for detail in details1]
		ans = [ans[0]+ans1[0]]
		#print(ans)
